...
public class ResidentService {
    ...
    public void mainMenu() {
        ...
        while (loggedIn) {
            ...
            System.out.println("6) View Requests Status History");
...
//        System.out.println("Logged in ID: " + user.getId());
    ...
    public void displayBalanceMenu() {
        try {
            ArrayList<ServiceRequest> requests = BackendService.getRequestsByStatusAndResident(resident.getId(), RequestStatus.approved);
    ...
    public void paySingleMenu(ServiceRequest request) {
        try {
            ...
            if (Character.toLowerCase(input) == 'y') {
                ...
                StatusHistory sH = new StatusHistory(request.getId(), RequestStatus.approved, RequestStatus.paid, "Request paid by resident.", resident.getName(), java.time.LocalDate.now().toString());
...
//                    int transactionId, String processedBy, PaymentMethod method, ArrayList<ServiceRequest> requests
    ...
    public void viewScheduledRequest() {
        ...
        try {
            ArrayList<Schedule> requests = BackendService.getScheduleByResidentName(resident.getName());
    ...
    public void displayStatusHistoryMenu() {
        ...
        try {
            ...
            int i = 1;
            for (ServiceRequest s : requests) {
                System.out.println(i++ + ") " + s.getServiceName() + " - " + s.getStatus() + " - " + s.getDateCreated());
            }
            ...
            if (option > 0 && option <= requests.size()) {
                ServiceRequest sr = requests.get(option - 1);
                resident.displayRequestStatusHistory(sr.getId());
            } else if (option == 0) {
                return;
            } else {
                System.out.println("Invalid selection.");
